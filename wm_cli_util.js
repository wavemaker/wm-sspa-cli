...
const printCliHeader = () => {
  ...
  if(process.env["BODA"] === "true") {
      log(
          ...
          text(` --build-once-deploy-anywhere <true/false> --project-path <local_project_path> --library-target <umd/system> --split-styles <true/false> --resource-hashing <true/false> --mount-styles <true/false>`)
      );
      log(
          textHead(`npx @wavemaker/wm-sspa-cli`),
      ...
      log(text(`Build Once Deploy Anywhere(-b): `), textHead(process.env.BODA));
      ...
      log(
          textHead(`Optional Params`),
          text(`build-once-deploy-anywhere(-b), library-target(-l : umd), split-styles(-c : false), resource-hashing(-r : false) & mount-styles(-m : true) take these specified default values if you don't specify anything`)
      );
      log("\n");
