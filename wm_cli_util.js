...
const printCliHeader = () => {
  ...
  if(process.env["PORTABLE_BUILD"] === "true") {
      log(
          ...
          text(` --portable-build <true/false> --project-path <local_project_path> --library-target <umd/system> --split-styles <true/false> --resource-hashing <true/false> --mount-styles <true/false>`)
      );
      log(
          textHead(`npx @wavemaker/wm-sspa-cli`),
          text(` -b true -p <local_project_path> -l umd -c false -m true -h true`)
      );
      log(
	  ...
	  //isCompatibleVersion();
      log(text(`Portable Build - Build Once Deploy Anywhere(-b): `), textHead(process.env.PORTABLE_BUILD));
      ...
      log(
          textHead(`Optional Params`),
          text(`portable-build(-b), library-target(-l : umd), split-styles(-c : false), resource-hashing(-r : false) & mount-styles(-m : true) take these specified default values if you don't specify anything`)
      );
      log("\n");
