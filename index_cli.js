...
if (argv["help"] || argv["h"]) {
  ...
  (async () => {
    ...
    if(argv["build-once-deploy-anywhere"] || argv["b"]) {
      argv["b"] = (argv["build-once-deploy-anywhere"] ? argv["build-once-deploy-anywhere"].toLowerCase() : (argv["b"] ? argv["b"].toLowerCase() : 'true'));
      if(!['true', 'false'].includes(argv['b'])) {
        printFailure(
            `Invalid value passed to build-once-deploy-anywhere(-b) "${argv["build-once-deploy-anywhere"] || argv["b"]}". Please check\n`
        );
        return;
      }
    } else {
    ...
    //build once deploy anywhere - flag
    process.env.BODA = argv["b"] || 'false';
